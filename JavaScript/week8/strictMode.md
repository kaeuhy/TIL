...
Strict Mode가 아닌 경우에는 매개변수와 `arguments` 객체가 내부적으로 서로 연결되어 있음

즉, 매개변수 `a` 와 `arguments[0]` 는 하나의 값을 함께 관리하기 때문에 어느 한쪽의 값을 변경하면 다른 쪽에도 동일하게 반영됨

→ 언제 어디에서 변경되었는지 추적하기 힘듦

</br>

Strict Mode에서는 매개변수와 `arguments` 객체를 서로 독립적으로 관리하도록 변경됨

그렇기에 `a = 2;` 를 수행하더라도 변경되는 것은 매개변수이지 `argument[0]` 에 전달된 초기 인수인 `1` 은 그대로 유지됨
